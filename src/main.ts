...
let expandedScenario: Scenario | null = null
...
function taxonomyMarkup() {
  const icons: Record<Scenario, string> = {
    scarce: 'ph-crown-simple',
    cashflow: 'ph-coins',
    decline: 'ph-warning-diamond',
  }
  const details: Record<Scenario, string[]> = {
    scarce: ['核心就业与公共服务半径', '供给难以复制、买方范围更广', '仍需核验成交价与具体瑕疵'],
    cashflow: ['成熟居住配套形成租金托底', '总价和月供处于家庭可承受区间', '收益来自使用价值而非单边上涨'],
    decline: ['新增同质供给削弱稀缺性', '远通勤与高总价压缩接盘人群', '需要更厚的价格折让与安全边际'],
  }

  return `<div class="section-intro lucr-section-intro" data-reveal><div><div class="overline">02 / ASSET SCENARIOS</div><h2>看清每一类资产</h2></div><p>先展开卡片理解资产逻辑，再把情景套入真实现金流。分类是压力测试，不是自动买卖建议。</p></div><div class="taxonomy-grid" data-reveal>${(Object.entries(scenarios) as [Scenario, typeof scenarios[Scenario]][]).map(([key, item], index) => {
    const expanded = expandedScenario === key
    return `<article class="taxonomy-card ${item.tone} ${expanded ? 'is-expanded' : ''}" data-scenario-card="${key}"><span class="taxonomy-index">0${index + 1}</span><i class="ph ${icons[key]} taxonomy-icon"></i><div class="taxonomy-tag">${item.eyebrow}</div><h3>${item.label}</h3><p>${item.description}</p><div class="taxonomy-data"><span>涨幅假设 <b>${item.appreciation > 0 ? '+' : ''}${item.appreciation}%</b></span><span>租售比参考 <b>${item.rentRatio}%</b></span></div><div class="scenario-detail" aria-hidden="${!expanded}"><ul>${details[key].map((detail) => `<li>${detail}</li>`).join('')}</ul><button data-scenario="${key}" aria-pressed="${key === activeScenario}"><span class="select-mark">${key === activeScenario ? '已套用 <i class="ph ph-check"></i>' : '套用此情景 <i class="ph ph-arrow-up-right"></i>'}</span></button></div><button class="scenario-expand" data-expand-scenario="${key}" aria-label="${expanded ? '收起' : '展开'}${item.label}" aria-expanded="${expanded}"><i class="ph ${expanded ? 'ph-x' : 'ph-plus'}"></i></button></article>`
  }).join('')}</div>`
}
...
function render() {
  ...
  const heroMapUrl = `${import.meta.env.BASE_URL}assets/beijing-map-hero.webp`
  document.querySelector<HTMLDivElement>('#app')!.innerHTML = `
  <header class="topbar lucr-topbar"><button class="nav-scrim" type="button" aria-label="关闭菜单"></button><a class="brand" href="#top" aria-label="资产分化首页"><span class="brand-mark"><i class="ph-bold ph-house-line"></i></span><span class="brand-name"><b>资产分化</b><small>ASSET DIVERGENCE LAB</small></span></a><nav id="mainNav"><a href="#model">决策模型</a><a href="#taxonomy">资产分类</a><a href="#selection">房源评分</a><a href="#method">方法与来源</a></nav><a class="topbar-cta" href="#model">开始测算</a><button class="menu-button" aria-label="打开菜单" aria-controls="mainNav" aria-expanded="false"><i class="ph ph-list menu-open-icon"></i><i class="ph ph-x menu-close-icon"></i></button></header>
  <main id="top"><section class="hero lucr-hero"><div class="hero-content section-shell"><div class="hero-copy"><h1><span>房子不再</span><br /><em>一起上涨</em></h1><p>把同一笔家庭现金流放进两条路径，找到真正适合你的北京住房资产。</p><div class="hero-actions"><button class="primary-button" id="startCalc">开始测算</button><a class="outline-button" href="#taxonomy">了解资产分类</a></div></div><figure class="hero-map-visual lucr-hero-map" aria-label="北京资产分化地图"><img src="${heroMapUrl}" alt="北京行政区立体城市模型" /></figure></div></section>
  <section class="model-section section-shell" id="model"><div class="lucr-chapter-heading" aria-hidden="true"><span>开始一次</span><span>真实测算</span></div><div class="calculator-heading"><div><div class="overline">01 / DECISION MODEL</div><h2>资产分化测算器</h2><p>输入真实条件，让两条现金流路径自己给出答案。</p></div><button class="save-button" id="saveReport" aria-label="导出报告"><i class="ph ph-download-simple"></i><span data-save-label>导出报告</span></button></div><div class="model-layout" data-reveal><aside class="input-panel"><div class="panel-head"><div><span class="eyebrow">INPUT VARIABLES</span><h3>输入你的真实方案</h3></div><button class="reset-button" id="resetInputs"><i class="ph ph-arrow-counter-clockwise"></i>重置</button></div><div class="input-group"><div class="group-title">核心参数 <span>单位：万元</span></div>${inputField('homePrice', '房屋总价', '万', '合同成交总价，不含税费和装修')}${inputField('downPayment', '首付金额', '万', '实际占用的首笔现金')}${inputField('monthlyRent', '同类月租金', '万', viewMode === 'self' ? '不买这套房时，同等居住体验的月租金' : '预计每月可收取的毛租金')}${inputField('income', '家庭月净收入', '万', '税后且可稳定用于家庭预算的月收入')}${inputField('years', '计划持有年限', '年', '模型按月计算，到期假设卖出')}</div><div class="input-group"><div class="group-title">市场假设 <span>不是预测</span></div>${inputField('appreciation', '房价年涨幅', '%', '允许负值，按复利计算')}<div class="growth-presets" aria-label="房价涨幅快捷情景"><button data-growth="-5">压力 −5%</button><button data-growth="0">持平 0%</button><button data-growth="3">温和 +3%</button></div>${inputField('wealthYield', '理财年收益率', '%', '替代投资的税费后年化收益')}${inputField('rentGrowth', '租金年变化', '%', '租金每满一年调整一次')}</div><details class="advanced-inputs"><summary><span><i class="ph ph-caret-right"></i>贷款、成本与压力参数</span><small>展开 9 项</small></summary><div class="input-group">${inputField('providentLoan', '公积金贷款', '万', '剩余贷款自动计入商贷')}${inputField('providentRate', '公积金利率', '%', '默认采用首套 5 年以上 2.6% 作为示例')}${inputField('commercialRate', '商业贷款利率', '%', `当前自动计算商贷 ${commercialLoan.toFixed(0)} 万`)}${inputField('loanYears', '贷款年限', '年', '等额本息贷款期限')}${inputField('purchaseCostRate', '买入税费/中介', '%', '按总价估算，可按实际情况修改')}${inputField('saleCostRate', '卖出成本', '%', '按卖出房产价值扣除')}${inputField('annualHoldingRate', '年持有成本', '%', '物业、维护与更新等年化估算')}${inputField('renovationCost', '装修与一次性支出', '万', '与首付一起计入初始机会成本')}${inputField('incomeDrop', '压力收入降幅', '%', '用于测算收入下降后的月供占比')}</div></details></aside><div class="results-panel" id="calculation-results">${resultsPanelMarkup(result)}</div><p class="sr-only" id="result-announcer" aria-live="polite"></p></div></section>
  <section class="taxonomy-section section-shell" id="taxonomy">${taxonomyMarkup()}</section>
  <section class="selection-section section-shell" id="selection">${selectionMarkup()}</section>
  <section class="method-section section-shell" id="method"><div class="section-intro lucr-section-intro"><div><div class="overline">04 / METHOD & SOURCES</div><h2>你能看见模型的边界</h2></div><p>本工具只在浏览器本地计算和保存，不上传你的收入、首付或房源评分。</p></div><div class="method-steps" aria-label="决策方法步骤"><article><i class="ph ph-chat-circle-dots"></i><b>01</b><span>输入家庭<br />真实现金流</span></article><article><i class="ph ph-magnifying-glass"></i><b>02</b><span>测算两条<br />财富路径</span></article><article><i class="ph ph-file-text"></i><b>03</b><span>运行压力<br />测试情景</span></article><article><i class="ph ph-sliders-horizontal"></i><b>04</b><span>评分具体<br />候选房源</span></article><article><i class="ph ph-check-circle"></i><b>05</b><span>保留证据<br />再做决定</span></article></div><div class="method-grid"><article><i class="ph ph-function"></i><h3>计算口径</h3><p>等额本息逐月摊还；租金与理财按月滚动；期末卖房净值扣除剩余贷款和卖出成本；临界涨幅用二分法求解。</p></article><article><i class="ph ph-database"></i><h3>政策与数据入口</h3><p>利率只是可编辑示例。资格、利率、税费和成交信息请在提交报价前从官方入口复核。</p><div class="source-links"><a href="https://gjj.beijing.gov.cn/web/zwgk61/2024zcwj/436433464/436433467/743903614/index.html" target="_blank" rel="noreferrer">北京公积金利率 <i class="ph ph-arrow-up-right"></i></a><a href="https://bjjs.zjw.beijing.gov.cn/eportal/ui?pageId=307749" target="_blank" rel="noreferrer">北京房地产数据 <i class="ph ph-arrow-up-right"></i></a><a href="https://zjw.beijing.gov.cn/bjjs/fdcjy/gfzg87/index.shtml" target="_blank" rel="noreferrer">购房资格核验 <i class="ph ph-arrow-up-right"></i></a></div></article><article class="disclaimer-card"><i class="ph ph-warning"></i><h3>风险声明</h3><p>结果不构成投资、贷款、税务或法律建议。模型不预测房价，也未纳入学区变化、重大维修、空置期、提前还款限制等所有个体因素。</p></article></div></section>
...
function refreshCalculator(syncInputs = true) {
  ...
  document.querySelectorAll<HTMLElement>('[data-scenario-card]').forEach((card) => card.classList.toggle('selected', card.dataset.scenarioCard === activeScenario))
  validateInputs()
  persistLocalState()
}
...
function bindEvents() {
  ...
  app.addEventListener('click', async (event) => {
    ...
    if (target.closest('.nav-scrim')) { setMenuState(false); return }
    const scenarioExpand = target.closest<HTMLButtonElement>('[data-expand-scenario]')
    if (scenarioExpand) {
      const key = scenarioExpand.dataset.expandScenario as Scenario
      expandedScenario = expandedScenario === key ? null : key
      const taxonomyRoot = document.querySelector<HTMLElement>('#taxonomy')
      if (taxonomyRoot) taxonomyRoot.innerHTML = taxonomyMarkup()
      return
    }
